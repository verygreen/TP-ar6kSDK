...
// 2010.10.11 HP Henry add
unsigned int enable_coexistence=0;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
module_param(enable_coexistence, int, 0644);
#else
MODULE_PARM(enable_coexistence,"i");
#endif

// 2010.10.22 HP Henry add
char macaddress[32]={0};
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
module_param_string(macaddress, macaddress, sizeof(macaddress), 0644);
#else
MODULE_PARM(macaddress,"s");
#endif

// 2010.12.20 HP Henry add
unsigned int enable_ota=0;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
module_param(enable_ota, int, 0644);
#else
MODULE_PARM(enable_ota,"i");
#endif
...
ar6000_init_module(void)
{
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_init_module\n"));
...
ar6000_cleanup_module(void)
{
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_cleanup_module\n"));
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_cleanup: success\n"));
}
...
void calculate_crc(A_UINT32 TargetType, A_UCHAR *eeprom_data, size_t eeprom_size)
{
    ...
    {
		// 2011.1.2 HP Henry modify
		// only calculate the first 1024 bytes for checksum
		printk("%s: AR6003 change eepromsize to 1024\n", __func__);
		eeprom_size = 1024;
...
ar6000_softmac_update(AR_SOFTC_T *ar, A_UCHAR *eeprom_data, size_t eeprom_size)
{
	...
	// 2010.10.22 HP Henry add: read mac address from "macaddress"
	if (macaddress[0])
	{
		char macbuf[20] = {0};
		unsigned int softmac[6];

		memcpy(macbuf, macaddress, 17);
		macbuf[17] = 0;
			...
			&softmac[3], &softmac[4], &softmac[5])==6) 
		{
				...
				source = "oem mac address";
		}
	}
	else if ((A_REQUEST_FIRMWARE(&softmac_entry, "softmac", ((struct device *)ar->osDevInfo.pOSDevice))) == 0)
        {
...
// 2011.3.10 HP Henry add: HP board data
// begin ==>
extern int wifi_get_board_type(char ** pPlatform, u32* pBoardType);
// <== end
...
ar6000_transfer_bin_file(AR_SOFTC_T *ar, AR6K_BIN_FILE file, A_UINT32 address, A_BOOL compressed)
{
    ...
    switch (file) {
...
#if HP_WORKAROUND_ART
				//hack hack
				if (bypasswmi)
					filename = "ath6k/AR6003/hw2.1.1/notp.bin";
				else
					filename = AR6003_REV3_OTP_FILE;
				//hack hack                 
#else
                filename = AR6003_REV3_OTP_FILE;
#endif // HP_WORKAROUND_ART
            } else {
        ...
        case AR6K_BOARD_DATA_FILE:
            if (ar->arVersion.target_ver == AR6003_REV2_VERSION) {
				...
				wifi_get_board_type(&pPlatform, &boardtype);
				printk("%s: platform is %s, boardtype is %d\n", __func__, pPlatform, boardtype);

				if (!strcmp(pPlatform, "topaz_wifi"))
					filename = AR6003_REV3_BOARD_DATA_FILE_TOPAZ_WIFI;
				else if (!strcmp(pPlatform, "topaz_3g"))
					filename = AR6003_REV3_BOARD_DATA_FILE_TOPAZ_3G;
				else if (!strcmp(pPlatform, "opal_wifi"))
					filename = AR6003_REV3_BOARD_DATA_FILE_OPAL_WIFI;
				else if (!strcmp(pPlatform, "opal_3g"))
					filename = AR6003_REV3_BOARD_DATA_FILE_OPAL_3G;
				else
					filename = AR6003_REV3_BOARD_DATA_FILE_TOPAZ_WIFI;
				// <== end
            } else {
	...
	AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("%s - fw: %s\n", __func__, filename)); 
...
ar6000_sysfs_bmi_get_config(AR_SOFTC_T *ar, A_UINT32 mode)
{
    ...
    if (mode == WLAN_INIT_MODE_UDEV) {
        ...
        if (ar->arTargetType == TARGET_TYPE_AR6003) {
			...
			if (bypasswmi) 
			{
				param =  AR6003_REV2_RAM_RESERVE_SIZE;                     
				bmifn(BMIWriteMemory(ar->arHifDevice,
...
#define CONFIG_ATH6KL_BT_UART_FC_POLARITY 0
        ...
        if (!eppingtest && bypasswmi) {
            ...
            AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("%s() ART mode\n", __func__));
...
ar6000_avail_ev(void *context, void *hif_handle)
{
    ...
    if(testmode) {
        ...
        AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_configure_target(tcmd)\n"));

        ar->arTargetMode = AR6000_TCMD_MODE;
    }else {

        AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_configure_target(wlan)\n"));
...
#ifdef HTC_RAW_INTERFACE
              if (!eppingtest && bypasswmi) {
                    AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_avail: ARTMODE\n"));
   ...
   for (i=0; i < num_device; i++)
   {
      ...
      AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000_avail: name=%s hifdevice=0x%lx, dev=0x%lx (%d), ar=0x%lx\n",
...
ar6000_restart_endpoint(AR_SOFTC_T *ar)
{
	...
	// 2011.1.13 HP Henry add
	if (!bypasswmi)
	{
		for(i = 0; i < num_device; i++) {
			arPriv = ar->arDev[i];

			if (arPriv->arWmiEnabled == FALSE)
			{
...
ar6000_destroy(struct net_device *dev, unsigned int unregister)
{
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("+ar6000_destroy \n"));
...
int ar6000_init(struct net_device *dev)
{
    ...
    A_INT32     timeleft;
	// 2010.10.11 HP Henry add
#if defined(INIT_MODE_DRV_ENABLED)    
...
//michael - leverage from new driver
#ifdef CONFIG_HOST_TCMD_SUPPORT //hack hack
    	if (ar->arTargetMode != AR6000_WLAN_MODE) {
        	AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("%s() TCMD mode\n", __func__));
        	goto skip;
    	}
...
// 2010.12.20 HP Henry modify
//#ifdef ATH6K_CONFIG_OTA_MODE
		if (enable_ota == 1)
		{
			AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("ar6000 : OTA MODE(set power MAX_PERF_POWER) \n"));

...
extern unsigned int enable_ota; // 2010.12.20 HP Henry add
...
A_STATUS ar6000_resume_ev(void *context)
{
	...
	// 2011/1/27 HP Henry add: keeping wake up for 4s, so WPA_SUPPLICANT can work normally.
	wake_lock_timeout(&ar6k_suspend_wake_lock, 4*HZ);
#endif
    return A_OK;
}
...
void plat_setup_power_stub(int on) 
{
	if (on)
	{
		wifi_set_power(1, 0);
//2010.12.17 HP Henry: don't call here, which will block suspend process!		wifi_set_carddetect(1);	/* CardDetect (0->1) */	
	}
	else
	{
		wifi_set_power(0, 0);
//2010.12.17 HP Henry: don't call here, which will block suspend process!		wifi_set_carddetect(0);	/* CardDetect (0->1) */	
	}
}
// <== end
...
static int ar6000_pm_probe(struct platform_device *pdev)
{
	...
	// 2010.12.17 HP Henry: move wifi_set_carddetect from plat_setup_power_stub to here!
...
//#endif
			...
			// 2011.2.22 HP Henry: add to fix deep sleep issue.
			// There are no pending events/irqs to handle. But "ar6k_wow_wake_lock" is not expired 
			// when DPM check wakelock status, which will cause resuming DUT. 
			// Call wake_unlock to free the wakelock(ar6k_wow_wake_lock).
			//printk("%s: call wake_unlock(&ar6k_wow_wake_lock) to allow entering suspend\n", __func__);
			wake_unlock(&ar6k_wow_wake_lock);
